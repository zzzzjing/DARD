...
def main():
    ...
    if os.path.exists(teacher_ckpt):
        ...
        print("未找到教师模型预训练参数，开始训练教师模型...")
        teacher_optimizer = optim.SGD(teacher_model.parameters(), lr=learning_rate,
                                      momentum=momentum, weight_decay=weight_decay)
        # 这里的学习率调度器及里程碑可根据需要调整
        teacher_scheduler = optim.lr_scheduler.MultiStepLR(teacher_optimizer, milestones=[10, 15], gamma=0.1)
        teacher_epochs = 20  # 为演示方便，教师模型训练 20 个 epoch；实际可适当增大
        for teacher_epoch in range(teacher_epochs):
            teacher_model.train()
            teacher_running_loss = 0.0
            for inputs, targets, _ in tqdm(trainloader_indexed, desc=f"教师模型训练 Epoch {teacher_epoch+1}/{teacher_epochs}"):
                inputs, targets = inputs.to(device), targets.to(device)
                teacher_optimizer.zero_grad()
                outputs = teacher_model(inputs)
                teacher_loss = F.cross_entropy(outputs, targets)
                teacher_loss.backward()
                teacher_optimizer.step()
                teacher_running_loss += teacher_loss.item() * inputs.size(0)
            teacher_scheduler.step()
            avg_teacher_loss = teacher_running_loss / len(trainset_indexed)
            print(f"教师模型训练 Epoch [{teacher_epoch+1}/{teacher_epochs}] Loss: {avg_teacher_loss:.4f}")
        teacher_acc = evaluate_clean(teacher_model, testloader, device)
        torch.save(teacher_model.state_dict(), teacher_ckpt)
        print("教师模型预训练完成，参数已保存。")
    teacher_model.eval()
