...
# 知识蒸馏 Dataset：返回图像、标签和教师对抗样本的 soft label
class DistillationDataset(Dataset):
    def __init__(self, base_dataset, teacher_adv):
        """
    ...
    def __getitem__(self, index):
        ...
        # 只使用教师模型对抗样本的 soft output，不融合 clean 样本的输出
        teacher_target = self.teacher_adv[index]
...
def main():
    ...
    # 构建知识蒸馏专用的数据集
    # 原始 CIFAR10 数据集中的 transform 已包含数据增强，本处不再额外 augment
    trainset = torchvision.datasets.CIFAR10(root='./data', train=True, download=False, transform=transform_train)
    # 只传入教师模型对抗样本的 soft output
    distill_dataset = DistillationDataset(trainset, teacher_adv_outputs)
    ...
    # 保存训练好的学生模型
    torch.save(student_model.state_dict(), "Only_teachersoftadv56_student18_distilled.pth")
