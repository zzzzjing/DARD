...
def pgd_attack(model, images, labels, epsilon, alpha, num_iter, mean, std, num_classes):
    """
    使用传统 PGD 攻击方法生成对抗样本，损失函数采用交叉熵损失。
    """
...
def main():
    ...
    for inputs, targets, indices in tqdm(trainloader_indexed, desc="教师模型推理"):
        ...
        # 对抗样本生成部分需要梯度
        adv_inputs = pgd_attack(teacher_model, inputs, targets, epsilon, alpha, num_iter, cifar10_mean, cifar10_std, num_classes)
        with torch.no_grad():
    ...
    # 构建知识蒸馏专用的数据集
    # 原始 IndexedCIFAR10 中的 transform 已包含数据增强，本处不再额外 augment
    ...
    # 学生模型对抗训练（使用 pgd_attack 生成对抗样本，然后匹配教师 soft label）
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets, teacher_soft in tqdm(distill_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            # 用学生模型自身生成对抗样本
            adv_inputs = pgd_attack(student_model, inputs, targets, epsilon, alpha, num_iter, cifar10_mean, cifar10_std, num_classes)
            optimizer.zero_grad()
    ...
    # 保存训练好的学生模型
    torch.save(student_model.state_dict(), "Alba_student_resnet18_distilled.pth")
