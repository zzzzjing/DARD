...
def main():
    ...
    # CIFAR-100 归一化参数（CIFAR-100 常用均值与标准差）
    ...
    trainset_indexed = IndexedCIFAR100(root='./data', train=True, download=True, transform=transform_train)
    ...
    # 初始化教师模型（ResNet56 for CIFAR100）并加载预训练权重（如果有）
    teacher_model = ResNet_CIFAR(BasicBlock, [9, 9, 9], num_classes=num_classes).to(device)
    teacher_ckpt = "teacher56_adv_CIFAR-100.pth"
    ...
    for inputs, targets, indices in tqdm(trainloader_indexed, desc="教师模型推理"):
        ...
        # 对抗样本生成部分需要梯度
        adv_inputs = mpgd_attack(teacher_model, inputs, targets, epsilon, alpha, num_iter, cifar100_mean, cifar100_std, num_classes)
        with torch.no_grad():
    ...
    # 构建知识蒸馏专用的数据集
    # 原始 IndexedCIFAR100 中的 transform 已包含数据增强，本处不再额外 augment
    ...
    # 保存训练好的学生模型
    torch.save(student_model.state_dict(), "student_resnet18_distilled_cifar100.pth")
