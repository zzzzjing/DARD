...
# 知识蒸馏 Dataset：返回图像、标签和教师 soft label, 融合 clean 与对抗版
class DistillationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 将 clean 与对抗教师输出融合（这里简单取平均，也可设计其他融合策略）
...
def cw_attack_eval(model, images, labels, c, kappa, steps, lr):
    # 利用 torchattacks 中的 CW 攻击
    adversary = torchattacks.CW(model, c=c, kappa=kappa, steps=steps, lr=lr)
...
def main():
    ...
    for inputs, targets, indices in tqdm(trainloader_indexed, desc="教师模型推理"):
        ...
        adv_inputs = mpgd_attack(teacher_model, inputs, targets, epsilon, alpha, num_iter, cifar10_mean, cifar10_std,
                                 num_classes)
        with torch.no_grad():
    ...
    # ----------------------------
    # 学生模型对抗训练（使用 mpgd_attack 生成对抗样本，然后匹配教师 soft label）
    # 同时结合真实标签的交叉熵损失以获得有效监督信号
    # ----------------------------
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets, teacher_soft in tqdm(distill_loader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            # 用学生模型自身生成对抗样本
            adv_inputs = mpgd_attack(student_model, inputs, targets, epsilon, alpha, num_iter, cifar10_mean,
                                     cifar10_std, num_classes)
            optimizer.zero_grad()
            outputs = student_model(adv_inputs)

            # 1. 知识蒸馏损失（温度平滑后的 KL 散度）
    ...
    # ----------------------------
    # 对抗攻击评估：Clean、FGSM、PGD、AutoAttack、CW
    # ----------------------------
    ...
    # PGD 攻击评估
    pgd_params = {'epsilon': epsilon, 'alpha': 2.0 / 255.0, 'num_iter': 20, 'cifar10_mean': cifar10_mean,
    ...
    # CW 攻击评估
    cw_params = {'c': 1e-4, 'kappa': 0, 'steps': 100, 'lr': 0.01}
    evaluate_attack(student_model, testloader, cw_attack_eval, cw_params, device, "CW")
